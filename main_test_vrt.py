...
def prepare_model_dataset(dictOne):
    ''' prepare model and dataset according to args.task. '''

    # define model
    if dictOne.get('task') == '001_VRT_videosr_bi_REDS_6frames':
        ...
        datasets = ['REDS4']
        dictOne['scale'] = 4
        dictOne['window_size'] = [6,8,8]
        dictOne['nonblind_denoising'] = False
    ...
    # download model
    model_path = 'model_zoo/vrt/001_VRT_videosr_bi_REDS_6frames.pth'
    ...
    # download datasets
    if os.path.exists(f'testsets/uploaded'):
        print(f'using dataset from testsets/uploaded')
    #else:
    #    if 'vimeo' in args.folder_lq.lower():
...
def test_video(lq, model, dictOne):
        ...
        if num_frame_testing:
            ...
            sf = dictOne.get('scale')
            num_frame_overlapping = dictOne.get('tile_overlap')[0]
            not_overlap_border = False
            b, d, c, h, w = lq.size()
            c = c - 1 if dictOne.get('nonblind_denoising') else c
